...
export default function Skills() {
  ...
  return (
    <>
      ...
      >
        {/* Title */}
  <div className="flex flex-row items-center justify-center gap-4 my-8 sm:my-10">
        ...
        <div
          ref={gridRef}
          className="w-full max-w-6xl mx-auto px-3 sm:px-4 grid grid-cols-4 md:grid-cols-5 gap-1.5 sm:gap-2 md:gap-3"
        >
          {Stack.map((skill, i) => (
            <div
              key={i}
              className="group relative aspect-square rounded-lg p-px bg-gradient-to-br from-white/10 to-white/0 hover:from-white/20 hover:to-white/5 transition-all duration-300"
            >
              <div
                className="skill-card relative h-full w-full rounded-[inherit] bg-black text-white shadow-sm shadow-black/10 flex items-center justify-center text-[10px] xs:text-xs sm:text-sm md:text-base lg:text-2xl text-center p-1.5 xs:p-2 sm:p-2.5 md:p-3 overflow-hidden transition-all duration-300 will-change-transform group-hover:-translate-y-0.5 group-hover:shadow-[0_8px_30px_rgba(255,255,255,0.12)] group-hover:scale-[1.02] after:content-[''] after:absolute after:inset-0 after:bg-[linear-gradient(120deg,transparent,rgba(255,255,255,0.08),transparent)] after:translate-x-[-100%] group-hover:after:translate-x-[100%] after:transition-transform after:duration-700"
              >
                {skill}
              </div>
            </div>
